...
(function () {
  if (window.__globalMenuToggleBound) return;
  window.__globalMenuToggleBound = true;

  document.addEventListener("click", function (e) {
    var button = e.target.closest(".ard-sosmed .hamburg");
    if (!button) return;

    var ard = button.closest(".ard-sosmed");
    if (!ard) return;

    var attentionWhore = ard.querySelector(".attention");
    var kontakMenu = ard.querySelector(".kontak-menu");

    if (kontakMenu && kontakMenu.classList.contains("active")) {
      kontakMenu.classList.remove("active");
      ard.classList.remove("kontak-active");

      setTimeout(function () {
        ard.classList.remove("open");

        if (attentionWhore) {
          attentionWhore.classList.add("whore");
        }
      }, 250);
    ...
    var isOpen = ard.classList.toggle("open");
  ...
  window.toggleKontak = function (event) {
    if (event) event.preventDefault();

    var ard = document.querySelector(".ard-sosmed");
    if (!ard) return;

    var menu = ard.querySelector(".kontak-menu");
    if (!menu) return;

    menu.classList.toggle("active");
    ard.classList.toggle("kontak-active", menu.classList.contains("active"));
  };
})();